        ...
        while len(buffer) > 0:
            ...
            for next_v in v.other_id:
                if next_v not in self.paths:
                    ...
                    if next_v != self:
                        if pth + 1 not in self.all_paths:
                            self.all_paths[pth + 1] = []
                        self.all_paths[pth + 1].append(next_v)
...
def hash(obj):
    return hash_inner([o.id for o in obj["pos"]], obj["opened"])
...
def hashes(obj):
    return [hash_inner([o.id for o in obj["pos"]], o) for o in sub_lists(obj["opened"])]
...
def path(valves):
    ...
    buffer = [{"pos": [v_map['AA'], v_map['AA']], "minute": 0, "flow": 0, "opened": []}]
    mflow = buffer[0]
    seen = {}
    ...
    while len(buffer) > 0:
        ...
        if current["flow"] >= mflow["flow"]:
            mflow = current
        # find all paths for each pos, at least one should be to gate!
        # find options for the other one (paths with length)
        for (i, cur) in enumerate(current["pos"]):
            for path in cur.paths:
