...
def fix_map(_map):
    half = len(_map) // 2
    _map[half - 1] = _map[half - 1][:half - 1] + '@#$' + _map[half - 1][half + 2:]
    _map[half] = _map[half][:half - 1] + '###' + _map[half][half + 2:]
    _map[half + 1] = _map[half + 1][:half - 1] + '%#&' + _map[half + 1][half + 2:]
    return _map
...
# PATH = (WHERE, LEN, KEYS_NEEDED, KEYS_GOTTEN)
# STACK = (LOCATIONS, STEPS, KEYS, PATH)
        ...
        for path in paths[start]:
            ...
            if path[0] not in curr[2]:
                if all([key in curr[2] for key in path[2]]):
                    ...
                    new_start = curr[0] + [path[0]]
                    new_start.remove(start)
                    if h not in best:
                        stack.append((new_start, curr[1] + path[1], new_keys, curr[3] + [path[0]]))
